...
export default function CalendarPage() {
  ...
  return <>
    ...
    <div className="dashboard-columns calendar-summary">
      <Card><SectionTitle title="다가오는 일정" description="내게 공개된 가까운 일정입니다." />{upcoming.length ? <div className="timeline-list">{upcoming.map((item) => <button key={item.id} onClick={() => setSelected(item)}><time>{item.startDate.slice(5).replace("-", ".")}</time><span><strong>{item.title}</strong></span>{item.isImportant && <Badge tone="red">중요</Badge>}</button>)}</div> : <EmptyState title="다가오는 일정이 없습니다." description="새 일정을 등록해 보세요." />}</Card>
