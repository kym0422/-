  ...
  return <>
    ...
    {tab === "mentors" ? <Card>
      ...
      {loading ? <p className="p-5 text-sm text-slate-500">멘토 배정 정보를 불러오는 중입니다.</p> : interns.length ? <div className="assignment-list">{interns.map((intern) => {
        ...
        return <article key={intern.id}><div className="table-user"><Avatar name={intern.name} /><span><strong>{intern.name}</strong><small>{intern.department ?? ""} · {intern.project_group ?? ""}</small></span></div><div className="assigned-mentors"><span><small>담당 멘토</small><strong>{primary?.name ?? "미배정"}</strong></span><span><small>서브 멘토</small><strong>{secondary?.name ?? "미배정"}</strong></span></div><Button variant="secondary" onClick={() => openAssignment(intern.id)}>배정 변경</Button></article>;
