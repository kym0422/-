...
export function WeeklyReportsPage({ mode }: { mode: Role }) {
  ...
  return <>
    ...
    {mode !== "INTERN" ? <Card className="selector-card"><SectionTitle title="인턴 선택" description={mode === "MENTOR" ? "본인에게 배정된 인턴만 표시합니다." : "현재 기수의 전체 인턴입니다."} /><div className="intern-selector">{interns.map((intern) => <button key={intern.id} className={activeInternId === intern.id ? "active" : ""} onClick={() => chooseContext(intern.id)}><Avatar imageUrl={intern.avatarUrl} name={intern.name} role={intern.role} /><span><strong>{intern.name}</strong><small>{intern.department}</small><em>{intern.projectGroup}</em></span></button>)}</div></Card> : null}
    <Card>
      <div className="report-context"><div>{activeIntern ? <><Avatar imageUrl={activeIntern.avatarUrl} name={activeIntern.name} role={activeIntern.role} size="large" /><span><small>{mode === "INTERN" ? "작성자" : "선택한 인턴"}</small><strong>{activeIntern.name}</strong><em>{activeIntern.department} · {activeIntern.projectGroup}</em></span></> : null}</div><div className="context-stats"><span><small>작성 주차</small><strong>{totals.weeks}</strong></span><span><small>전체 항목</small><strong>{totals.items}</strong></span></div></div>
