...
export function TasksPage({ mode }: { mode: Role }) {
  ...
  return <>
    ...
    <Card><SectionTitle title="과제 타임라인" description={totalWeeks + "주 실습 기간의 과제 일정을 확인합니다."} />{loading ? <p className="p-5 text-sm text-slate-500">과제를 불러오는 중입니다.</p> : tasks.length ? <div className="task-timeline"><div className="timeline-header"><span>과제</span>{weekCells.map((week) => <span key={week}>{week}주</span>)}</div>{tasks.map((task) => <div className="timeline-row" key={task.id}><div><strong>{task.title}</strong><small>{categoryLabel(task.primaryCategory, task.primaryCategoryCustom)} · {difficultyLabels[task.difficulty]}</small></div>{weekCells.map((week) => <span key={week} className={week >= task.startWeek && week <= task.endWeek ? "filled" : ""}>{week === task.startWeek ? <i>{task.startWeek}–{task.endWeek}주</i> : null}</span>)}</div>)}</div> : <EmptyState title="배정된 과제가 없습니다." description={mode === "INTERN" ? "새 과제가 배정되면 여기에 표시됩니다." : "첫 과제를 배정해 보세요."} />}</Card>
    {!loading ? <div className="task-list">{tasks.map((task) => <Card className="task-card" key={task.id}><div className="task-card-header"><span className="task-number"><CalendarRange size={19} /></span><div><span><Badge tone={task.difficulty === "ADVANCED" ? "red" : task.difficulty === "INTERMEDIATE" ? "amber" : "green"}>{difficultyLabels[task.difficulty]}</Badge><Badge tone="blue">{task.startWeek}–{task.endWeek}주</Badge></span><h2>{task.title}</h2></div>{mode !== "INTERN" ? <div><button className="icon-button" onClick={() => openEdit(task)} aria-label="과제 수정"><Pencil size={17} /></button><button className="icon-button danger-icon" onClick={() => void remove(task)} aria-label="과제 삭제"><Trash2 size={17} /></button></div> : null}</div><p>{task.summary}</p><dl><dt>1차 분류</dt><dd>{categoryLabel(task.primaryCategory, task.primaryCategoryCustom)}</dd><dt>2차 분류</dt><dd>{secondaryCategoryLabel(task.secondaryCategory, task.secondaryCategoryCustom)}</dd><dt>기대 산출물</dt><dd>{outputLabel(task.expectedOutput, task.expectedOutputCustom)}</dd></dl><footer>배정자 {data.profiles.find((profile) => profile.id === task.assignedBy)?.name ?? "운영자"} · {displayDate(task.createdAt)}</footer></Card>)}</div> : null}
